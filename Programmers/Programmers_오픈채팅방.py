...
def solution(record: List[str]) -> List[str]:
    ...
    for msg in record:
        ...
        if len(split_msg) > 2:
            nick_table[split_msg[1]] = split_msg[2]

    for msg_idx in range(len(store_list)):
        nickname = nick_table[store_list[msg_idx].split(' ')[1]]
        if store_list[msg_idx][0] == 'E':
            answer.append( nickname+"님이 들어왔습니다.")
        elif store_list[msg_idx][0] == 'L':
